...
read_data <- function(intensity_data, delimiter) {
    df <- read.csv(intensity_data, sep=delimiter, row.names=1, check.names=FALSE)
    return(df)
}
...
make_biplot <- function(metadata, pca_results) {
    ...
    plot_df <- left_join(scores, meta, by="geo_accession")
    
    ggplot(plot_df, aes(x=PC1, y=PC2, color=SixSubtypesClassification)) +
      geom_point() +
      labs(title="PCA Biplot: PC1 vs PC2", x="PC1", y="PC2", 
           color="Six Subtypes Classification") +
      theme_bw()
}
